...
> v11.20: 强化命名规范+表格对齐 — 禁止单字国名缩写, 表格列宽须目测 — openfootball 2026 6/11-6/22是真实数据(93%验证通过)
...
### 国家名
- **国家名必须使用全称**，全文禁止任何单字缩写
- 禁止: 哥(哥伦比亚)、苏(苏格兰)、葡(葡萄牙)、英(英格兰)、克(克罗地亚)、巴(巴西/巴拿马)、摩(摩洛哥)、加(加拿大/加纳)、日(日本)、韩(韩国)、刚(刚果) 等一切单字简称
- 正确: 哥伦比亚、苏格兰、葡萄牙、英格兰、克罗地亚、巴西、巴拿马、摩洛哥、加拿大、加纳、日本、韩国、刚果民主共和国(刚果金)
- 唯一例外: C罗(克里斯蒂亚诺·罗纳尔多的通用简称)、B费(布鲁诺·费尔南德斯)
- ⚠️ **批量替换警告**: 摩洛哥墨西哥乌兹别克捷克等词内含有哥克字。用Python替换时必须用正则词边界或手动指定上下文，禁止纯字符串replace。6/24复盘文件因此导致摩洛哥→摩洛哥伦比亚等23处破词。
...
### .md 完整版排版
1. 标题层级: `##` 主标题 -> `###` 比赛标题 -> `####` 小节
2. **表格列宽必须对齐**: 每列宽度 = 该列最长内容 + 2空格缓冲。特别关注:
   - 时间列: `| 时间 |` 至少预留到 `| 90'+5 |` 的宽度
   - 球员名列: 中文名最长可达7字(如"克里斯蒂亚诺·罗纳尔多"), 必须给足空间
   - 方式列: 描述进球方式(如"左脚近距离（斯塔尼希奇传中助攻）"), 需足够宽
   - **生成表格后必须目测: 如果某列内容被挤到换行, 就加宽该列**
...
> ⚠️ 6/24复盘教训提炼过程犯了两个错误:
>   ① 先写教训再找数据 → 写了"大巴守不住75分钟"但没验证
>   ② 查数据时只验证了1场(哥伦比亚vs刚果)就宣布全假 → 实际93%是真的。过度修正。

**教训提炼流程 (按顺序, 不准跳步):**

```
[1] 先确认数据源可靠: ESPN API = 可信, openfootball 2026 (=<6/22) = 可信, 6/23+需验证
    → 验证方法: 用ESPN API交叉对比, 不是只查1场就下结论
[2] 用可靠数据验证假设
[3] 数据说啥就写啥: 如果数据不支持"直觉", 修改直觉
[4] 标注数据来源和日期范围: 如 "openfootball 6/11-6/22, N=31"
[5] 没有可靠数据就不写: 写成"待验证假设", 不要写成结论
```
...
| 陷阱 | 案例 (6/24) | 为什么是错的 |
|------|-----------|------------|
| "X为全天定了调" | 英格兰0-0加纳 → 全天基调 | 每场独立，没有因果关系。一场比赛的结果不影响同天另一场。 |
| "A队和B队面临相同困境" | 英格兰=哥伦比亚, 都是体系型被非洲大巴 | 每场独立。英格兰无金特罗, 哥伦比亚有。不能因为标签相同就默认结果相同。 |
| "历史在重演" | — | 除非有可靠数据证明, 否则不准用。即使是真实规律也要标注样本量。 |
...
### ⚠️ openfootball 2026 数据状态

| 日期范围 | 状态 | 验证 |
|---------|------|------|
| 6/11 - 6/22 | ✅ 真实数据 | 14/14 spot check vs ESPN API 全对 |
| 6/23 | ⚠️ 基本正确，1个错误 | 哥伦比亚vs刚果: JSON 0-0, ESPN 1-0 (76'进球后未更新) |
| 6/24+ | ❌ 全部0-0占位符 | `git log`: auto-gen week 26/3, 尚未更新 |

> **规则: 使用 openfootball 2026 数据前必须确认日期 ≤ 6/22。6/23需交叉验证ESPN。6/24+不可用。**

### 大巴-75分钟规律 (openfootball 6/11-6/22 真实数据, N=5)

| 0-0到75' | 最终0-0 | 最终破门 |
|----------|---------|---------|
| **5场** | **3场 (60%)** | **2场 (40%)** |

> ⚠️ 样本仅5场，不足以形成统计规律。但破门2场都是**非洲球队自己绝杀**(科特迪瓦90'、加纳90+5')——非洲大巴不只守得住，还有能力在最后时刻偷胜。

### 2026 vs 2022 冷门率对比 (openfootball 真实数据)

| | 2022 世界杯 | 2026 世界杯 |
|------|----------|----------|
| 亚非弱队冷门率 | **56%** (15/27) | **55%** (17/31) |
| 弱队赢球 | 41% | 19% |
| 平局 | 15% | 35% |

> 冷门率几乎相同。但2026弱队更倾向求平(35% vs 15%), 大巴战术成为标配。

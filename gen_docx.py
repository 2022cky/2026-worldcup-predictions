...
import json, os, sys
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "2026年6月28日_6场预测_v2.docx")
...
summary_data = [
    ...
    ['1', '05:00', 'L', 'CRO 克罗地亚 vs GHA 加纳', '克3分/加4分 生死战', '1-1', '0-0', '克1-0 / 加1-0', '中'],
    ['2', '05:00', 'L', 'PAN 巴拿马 vs ENG 英格兰', '巴淘汰/英4分', '英格兰 3-0', '1-0', '2-0 / 4-0', '极低'],
    ['3', '07:30', 'K', 'COL 哥伦比亚 vs POR 葡萄牙', '哥6分/葡4分 争头名', '1-1', '0-0', '葡2-1 / 哥1-0', '低'],
    ['4', '07:30', 'K', 'COD 刚果金 vs UZB 乌兹别克', '刚1分/乌淘汰', '刚果金 2-0', '1-0', '1-0 / 3-0', '低'],
    ['5', '10:00', 'J', 'ALG 阿尔及利亚 vs AUT 奥地利', '双方3分 争第2', '0-0', '0-0', '奥1-0 / 阿1-0', '中高'],
    ['6', '10:00', 'J', 'JOR 约旦 vs ARG 阿根廷', '约淘汰/阿6分头名', '阿根廷 3-0', '2-0', '2-0 / 4-0', '极低'],
]
...
groups_data = {
    'L组': [
        ...
        ['1', '英格兰', '2', '1', '1', '0', '4/2', '+2', '4', '[V]'],
        ['2', '加纳', '2', '1', '1', '0', '1/0', '+1', '4', '[V]'],
...
bracket_data = [
    ...
    ['L', '英格兰/加纳', '加纳/克罗地亚', '3rd-place [可]', '葡萄牙(K2) [危]', '可打', '死亡级'],
    ['K', '哥伦比亚/葡萄牙', '葡萄牙/哥伦比亚', '3rd-place [可]', 'L2(英/加/克) [注意]', '可打', '中等'],
    ['J', '阿根廷(已锁)', '奥/阿争', '3rd-place [可]', 'ESP 西班牙(H1) [死亡]', '可打', '死亡级'],
]
...
for i, row in enumerate(bracket_data[1:], 1):
    for j, text in enumerate(row):
        ...
        color = RED_BG if '[死亡]' in text else None
...
matches = [
    {
        ...
        'home': '克罗地亚', 'away': '加纳',
        'home_flag': 'CRO', 'away_flag': 'GHA',
        'situation': '克罗地亚3分(须赢才稳) / 加纳4分(打平出线)  |  L2=葡萄牙[死亡]→双方都想避免',
        'venue': '—',
        'factors': [
            ('加纳已验证大巴(0-0英格兰)', '加纳 [火][火]', '加纳大巴是本届顶级 — 英格兰78%控球=0进球'),
            ('淘汰赛路径: L2=葡萄牙[危]', '双方都不想第2', '打平=加纳L1→安全路径'),
            ('莫德里奇回归首发', '克罗地亚 [火]', '前两场轮换→本场全主力'),
            ('克罗地亚防线老化(4失球)', '加纳', '速度型反击是克罗地亚最怕的'),
            ('加纳自主进攻极弱(2场1球)', '克罗地亚', '加纳0-0英格兰=极品大巴但00进攻'),
        ],
        'strength': '克罗地亚: 体系型(莫德里奇单人超巨年龄折扣)  |  加纳: 大巴型(已验证0-0英格兰)',
        'africa_resilience': [('低位防守', '[对][对] 0-0英格兰'), ('速度反击', '[对][对] 耶伦基'), ('前30分钟', '[对][对] 2场均不丢球'), ('被压制不崩盘', '[对][对] 对英格兰全场'), ('定位球', '[对]')],
    ...
    },
    {
        ...
        'home': '巴拿马', 'away': '英格兰',
        'home_flag': 'PAN', 'away_flag': 'ENG',
        'situation': '巴拿马0分已淘汰 / 英格兰4分(打平即头名)  |  身价比 英格兰€1.31B vs 巴拿马~€15M ≈ 87:1',
        'venue': '—',
        'factors': [
            ('身价比87:1', '英格兰 [火][火][火]', '极端差距—强队方不做小比分预测'),
    ...
    },
    {
        ...
        'home': '哥伦比亚', 'away': '葡萄牙',
        'home_flag': 'COL', 'away_flag': 'POR',
        'situation': '哥伦比亚6分(Q) / 葡萄牙4分(Q) — 双方均已出线，争头名  |  身价 葡萄牙€957M vs 哥伦比亚~€250M',
        'venue': '—',
        'factors': [
            ('哥伦比亚打平即头名', '哥伦比亚 [火]', '平局=K1=3rd路径[可]→不冒险'),
            ('葡萄牙必须赢才能头名', '哥伦比亚 [火]', '葡压上→哥反击→萨拉赫式剧本'),
            ('身价差3.8:1(葡萄牙€957M)', '葡萄牙 [火]', 'C罗+B费+莱昂=超级巨星型'),
    ...
    },
    {
        ...
        'home': '刚果金', 'away': '乌兹别克',
        'home_flag': 'COD', 'away_flag': 'UZB',
        'situation': '刚果金1分(可争第3出线) / 乌兹别克0分已淘汰  |  刚果金赢球=4分第3大概率晋级',
        'venue': '—',
        'factors': [
            ('刚果金防守验证(仅2失球对葡+哥)', '刚果金 [火][火]', '姆本巴+图安泽贝=世界杯级防线'),
            ('乌兹别克-7净胜球+0分已淘汰', '刚果金 [火]', '已无动力→荣誉战积极性存疑'),
            ('刚果金赢球可能出线(4分第3)', '刚果金 [火]', '有明确目标→动力碾压'),
            ('乌兹别克对葡萄牙5球惨败', '刚果金', '防线薄弱→刚果金可轻松破门'),
        ],
        'strength': '刚果金: 防守型(对比葡仅丢2球)  |  乌兹别克: 不适用',
        'africa_resilience': [('低位防守', '[对][对] 对比葡仅丢2球'), ('速度反击', '[对] 维萨'), ('前30分钟', '[对] 均未丢球')],
    ...
    },
    {
        ...
        'home': '阿尔及利亚', 'away': '奥地利',
        'home_flag': 'ALG', 'away_flag': 'AUT',
        'situation': '双方均3分 — 争J组第2  |  [!] J2 = 西班牙(€1.27B) = 死亡路径! 双方可能都不想赢!',
        'venue': '—',
        'factors': [
            ('[!] J2=西班牙(€1.27B)=死刑路径', '双方都不想第2! [火][火][火]', 'J2对西班牙=提前出局——类似挪威放弃争头名'),
...
add_para('[注意] 首发阵容尚未官方确认(距开球>4小时)  |  预测仅供分析参考  |  生成工具: python-docx',
         size=Pt(7), color=RGBColor(0xAA,0xAA,0xAA), align='center')
